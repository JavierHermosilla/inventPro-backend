...
function tableRowFixed (doc, cols, values, options = {}) {
  ...
  const y = doc.y
  let x = MARGIN_L
  if (bg) { doc.save(); doc.rect(MARGIN_L, y - 1, CONTENT_W, ROW_H + 2).fill(bg).restore() }
  if (bold) doc.font('Helvetica-Bold')
  if (color) doc.fillColor(color)
  doc.fontSize(10)
  for (let i = 0; i < cols.length; i++) {
    const c = cols[i]
    const v = fit(doc, values[i], c.width) // ← evita wrap
    doc.text(v, x + 6, y + 2, { width: c.width - 12, align: c.align || 'left', lineBreak: false })
    x += c.width
  }
  doc.strokeColor('#eee').lineWidth(0.5).moveTo(MARGIN_L, y + ROW_H).lineTo(MARGIN_R, y + ROW_H).stroke()
  doc.fillColor('black')
...
export async function exportFullInventoryPDF (req, res) {
  try {
    const { products, suppliers, clients, orders, inventoryUnits, inventoryValue, totalOrders } = await loadDatasets()

    res.setHeader('Content-Type', 'application/pdf')
    res.setHeader('Content-Disposition', `attachment; filename="full_inventory_${new Date().toISOString().replace(/[:.]/g, '-')}.pdf"`)
    ...
    // ---------- Productos (columnas simétricas: suma 495) ----------
    ...
    for (const s of suppliers) {
      ...
      withReflowedHeader(doc, COLS_SUP, () => {
        tableRowFixed(doc, COLS_SUP, [s.name || '', s.rut || '', s.phone || '', s.email || ''], { bg: zebra ? '#fcfcfc' : null })
      })
    }
    ...
    const COLS_CLI = [
      ...
      { header: 'RUT', width: 70, align: 'left' },
      { header: 'Teléfono', width: 90, align: 'left' },
      { header: 'Email', width: 120, align: 'left' },
      { header: 'Dirección', width: 50, align: 'left' }
    ]
    ...
    for (const c of clients) {
      ...
      withReflowedHeader(doc, COLS_CLI, () => {
        tableRowFixed(doc, COLS_CLI, [c.name || '', c.rut || '', c.phone || '', c.email || '', c.address || ''], { bg: zebra ? '#fcfcfc' : null })
      })
    }
    ...
    const COLS_ORD = [
      ...
      { header: 'Cliente', width: 125, align: 'left' },
      { header: 'Estado', width: 60, align: 'center' },
      { header: 'Backorder', width: 50, align: 'center' },
      { header: 'Ítems', width: 35, align: 'right' },
    ...
    for (const o of orders.slice(0, 10)) {
      ...
      withReflowedHeader(doc, COLS_ORD, () => {
        ...
        if (itemsCount) {
          ...
          // detalle en línea siguiente alineado a partir de la 3a columna
...
export async function exportFullInventoryCSV (req, res) {
  try {
    ...
    const mode = String(req.query.mode || 'zip').toLowerCase()
    ...
    const suppliersRows = suppliers.map(s => ({ name: s.name, rut: s.rut || '', phone: s.phone || '', email: s.email || '' }))
    const clientsRows = clients.map(c => ({ name: c.name, rut: c.rut || '', phone: c.phone || '', email: c.email || '', address: c.address || '' }))
    ...
    if (mode === 'merged') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="full_inventory_${new Date().toISOString().replace(/[:.]/g, '-')}.csv"`)
      res.end(csv)
      return
    }
    ...
    res.setHeader('Content-Disposition', `attachment; filename="full_inventory_${new Date().toISOString().replace(/[:.]/g, '-')}.zip"`)
...
export async function exportFullInventoryXLSX (req, res) {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="full_inventory_${new Date().toISOString().replace(/[:.]/g, '-')}.xlsx"`)
    ...
    wsC.addRows(clients.map(c => ({ name: c.name, rut: c.rut || '', phone: c.phone || '', email: c.email || '', address: c.address || '' })))
